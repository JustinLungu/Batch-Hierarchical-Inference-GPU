...
# Configuration: Scripts for starting Edge Device and Edge Server
EDGE_DEVICE_START_SCRIPT = "app/edge_device/edge_device.py"
EDGE_SERVER_START_SCRIPT = "app/edge_server/edge_server.py"

# Configuration: Experiment parameters
experiment_config = {

    # Configuration: Experiment parameters
    "sample_path": "data/datasets/imagenette/val_renamed", # Loaded dataset
...
# Function: Start Edge Device and Edge Server and wait for them to be started
def start_server(script_name, server_url, timeout=30):
    print(f"Starting {script_name} on url: {server_url}...")
    command = ["python", script_name, "--no-reload"]
    ...
    while time.time() - start_time < timeout:
        try:
            ...
            if response.status_code == 200:
                print(f"{script_name} running on url: {server_url}.")
                return process
        except requests.exceptions.ConnectionError:
            time.sleep(1)
    print(f"Error: {script_name} did not start in {timeout} seconds.")
    process.terminate()
    return None
...
# Start Edge Server and Edge Device
edge_server_process = start_server(EDGE_SERVER_START_SCRIPT, EDGE_SERVER)
if not edge_server_process:
    print("Edge Server failed to start. Exiting...")
    exit(1)
edge_device_process = start_server(EDGE_DEVICE_START_SCRIPT, EDGE_DEVICE)
if not edge_device_process:
    print("Local Device Server failed to start. Exiting...")
    edge_server_process.terminate()
    exit(1)

# Main function to run the experiment
if __name__ == "__main__":
    send_experiment_config()
...
# Shutdown servers when complete
print("Stopping servers...")
if edge_server_process:
    edge_server_process.terminate()
if edge_device_process:
    edge_device_process.terminate()
