...
if [[ -n "${PASS:-}" ]]; then
    ...
    log "Warning: PASS environment variable not set. Root SSH access may be blocked."
fi
service ssh start

# Run the Edge Server application
log "Starting edge_server.py"
exec python edge_server/edge_server.py
