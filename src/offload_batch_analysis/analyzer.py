...
class OffloadBatchAnalyzer:
    ...
    def extract_batch_measurements(
        ...
        for batch_id, (_, batch) in enumerate(grouped):
            received_at = single_batch_value(
                batch, "ts_sample_received_at_edge_server", batch_id
            )
            results_sent_at = single_batch_value(
                batch, "ts_results_sent_to_edge_device", batch_id
            )
            lml_start = minimum_batch_value(
                batch, "ts_lml_inference_start", batch_id
            )
            lml_end = maximum_batch_value(batch, "ts_lml_inference_end", batch_id)
    ...
    def summarize_by_batch_size(
        ...
        for (config_id, controller_size, actual_size), group in measurements.groupby(
            ...
            response = numeric_series(group, "server_response_time_s")
            per_image = numeric_series(group, "per_image_server_time_s")
            throughput = numeric_series(
                group, "effective_server_throughput_samples_s"
            )
            lml_wall = numeric_series(group, "lml_wall_time_s")
            micro_batches = numeric_series(group, "lml_micro_batches_observed")
            rows.append(
                {
                    "config": config_id,
                    ...
                    "per_image_time_std_s": per_image.std(),
    ...
    def calculate_config_trends(
        ...
        for config_id, group in measurements.groupby("config", sort=True):
            batch_size = numeric_series(group, "actual_server_batch_size")
            response = numeric_series(group, "server_response_time_s")
            per_image = numeric_series(group, "per_image_server_time_s")
            throughput = numeric_series(
                group, "effective_server_throughput_samples_s"
            )
            slope, r_squared = linear_trend(batch_size, response)
            rows.append(
                {
                    "config": config_id,
                    "controller_batch_size": int(
                        single_numeric_value(group, "controller_batch_size")
                    ),
